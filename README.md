...
一个功能强大的Android设备测试和日志管理工具，基于PySide6构建的现代化界面，支持实时日志过滤、MTKLOG管理、网络信息监控等多种功能。
...
**主要功能**:
- **MTKLOG控制**: 开启/停止MTKLOG、设置日志大小、切换SD/USB模式、安装MTKLOGGER、启用Telephony日志
- **ADB Log控制**: 连线log（实时显示）、离线log（后台保存）、导出日志、TCPDUMP抓包
- **Google日志**: 启用/停止Google日志收集
- **Bugreport**: 生成、拉取、删除系统bugreport
- **AEE Log**: 启动AEE异常日志收集
- **日志操作**: 合并MTKlog、从MTKlog提取pcap、合并PCAP、从高通log提取pcap、3GPP解码器、MTK SIP解码

**使用提示**: 
- 开启MTKLOG前确保设备有足够存储空间
- 连线log会实时显示在日志区域，离线log在后台保存
- 使用TCPDUMP前需要root权限
...
**主要功能**:
- **关键字过滤**: 支持正则表达式和普通关键字过滤
- **过滤选项**: 正则表达式模式、区分大小写、彩色高亮
- **日志管理**: 管理log关键字、设置显示行数、保存日志、清除缓存

**使用提示**:
- 输入关键字后按回车或点击"开始过滤"即可开始
- 支持正则表达式，如 `error|warning` 可同时匹配多个关键字
- 过滤过程中可以随时点击"停止过滤"停止

---

#### 🌐 网络信息
**功能概述**: 实时监控设备网络状态和连接信息

**主要功能**:
- **网络信息监控**: 实时显示SIM卡、RAT、频段、ARFCN、PCI、RSRP、RSRQ、SINR、RSSI等网络参数
- **Ping测试**: 网络连通性测试，显示网络状态

**使用提示**:
- 点击"开始"按钮开始监控，网络信息会实时更新在表格中
- 支持多SIM卡和多载波聚合（CA）信息显示
- Ping测试用于快速检查网络连接状态

---

#### 📱 APP操作
**功能概述**: 应用安装、查询、管理和进程控制

**主要功能**:
- **查询操作**: 查询package列表、查询包名、查询安装路径
- **APK操作**: pull apk（从设备拉取）、push文件（推送到设备）、安装APK
- **进程操作**: 查看进程列表、dump app（导出应用信息）
- **APP状态操作**: 启用/禁用应用

**使用提示**:
- 查询操作需要输入包名或应用名称
- pull apk会自动保存到本地log目录
- 禁用应用后需要重新启用才能使用

---

#### 📲 SIM
**功能概述**: SIM卡APDU消息解析和SIM卡数据读写

**主要功能**:
- **SIM APDU解析器**: 
  - 支持MTK原始日志、MTK表格格式和通用APDU文本格式
  - 自动识别eSIM消息、CAT命令和普通SIM卡消息
  - 详细的TLV结构解析和图形化展示
  - 支持正则表达式搜索和过滤
- **SIM卡读写工具**:
  - 读取SIM卡EF文件数据（IMSI、ICCID、IMPI、IMPU等）
  - 写入SIM卡数据（单条记录或批量JSON文件）
  - EF文件管理（创建、删除、结构调整）
  - 智能端口选择和自动连接

**使用提示**:
- APDU解析器需要先加载日志文件或APDU文本
- SIM卡读写工具需要选择正确的串口并输入PIN码
- 批量写入支持JSON格式配置文件

---

#### 📁 TMO CC
**功能概述**: TMO CC文件管理和日志过滤（TMO专用功能）

**主要功能**:
- **CC文件操作**: 推CC文件到设备、从设备拉CC文件
- **服务器选择**: 切换PROD服务器或STG服务器
- **过滤操作**: 简单过滤、完全过滤、清空日志、清除手机缓存日志
...
#### 📞 TMO Echolocate
**功能概述**: Echolocate文件管理和日志过滤（TMO专用功能）

**主要功能**:
- **Echolocate操作**: 安装、Trigger触发、Pull文件、删除手机文件、查看版本
- **过滤操作**: CallID、CallState、UICallState、AllCallState、IMSSignalling、AllCallFlow、voice_intent测试

**使用提示**:
- 需要先安装Echolocate应用
- Trigger会触发Echolocate收集数据
- 各种过滤按钮用于快速过滤特定类型的日志

---

#### ⏰ 24小时背景数据
**功能概述**: 24小时背景数据测试配置和日志分析（TMO专用功能）

**主要功能**:
- **配置手机**: 配置设备进行24小时背景数据测试
- **分析log**: 分析收集的背景数据日志
...
#### 🛠️ 其他
**功能概述**: 设备信息查询、工具配置和各种辅助功能

**主要功能**:
- **设备信息**: 查看手机信息（IMEI、ICCID、IMSI等）、设置灭屏时间、暗码管理、高通lock cell、高通NV
- **赫拉配置**: 赫拉配置、赫拉测试数据收集
- **其他操作**: 
  - 输入文本（向设备输入文本）
  - AT工具（发送AT命令）
  - PR翻译（将中文PR翻译成英文Word文档）
  - 转码工具（ASCII和GSM 7-bit编码转换）
  - 日志区域行数设置
  - 工具配置（在线更新、下载目录等）
  - 配置备份恢复（导出/导入工具配置）
  - 自定义界面管理（Tab管理、自定义按钮管理）

**使用提示**:
- 手机信息查询会显示设备的详细信息
- 工具配置中可以设置在线更新相关参数
- 自定义界面管理可以创建自定义Tab和按钮，灵活配置界面
...
**功能说明**：
- 自动执行 `scripts\build.bat`、压缩 onedir 目录、计算 SHA256 并生成 `releases/latest.json`
