...
class GoogleLogWorker(QThread):
    ...
    def __init__(self, device, operation, google_log_folder=None):
        super().__init__()
        self.device = device
    ...
    def _start_google_log(self):
        ...
        try:
            # 1. 创建日志目录
            self.progress.emit("创建Google日志目录...", 10)
            ...
            # 2. 执行Google日志相关命令
            self.progress.emit("配置Google日志设置...", 20)
            ...
            # 前两个命令直接执行
            for i in range(2):
                self.progress.emit(f"执行命令 {i+1}/2...", 20 + (i * 5))
            ...
            with open(other_info_file, 'w', encoding='utf-8') as f:
                for i in range(2, len(commands)):
                    self.progress.emit(f"收集设备信息 {i-1}/{len(commands)-2}...", 30 + ((i-2) * 8))
                ...
                for i in range(6, len(commands)):
                    self.progress.emit(f"设置日志级别 {i-5}/{len(commands)-6}...", 60 + ((i-6) * 3))
                    subprocess.run(commands[i], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30, 
                                 creationflags=subprocess.CREATE_NO_WINDOW if hasattr(subprocess, 'CREATE_NO_WINDOW') else 0)
            
            # 3. 启动ADB日志
            self.progress.emit("启动ADB日志...", 80)
            ...
            # 4. 启动视频录制
            self.progress.emit("启动视频录制...", 90)
            
            # 注意：同上，需要通过主线程调用video_manager
            
            # 5. 完成
            self.progress.emit("Google日志收集已启动!", 100)
    ...
    def _stop_google_log(self):
        ...
        try:
            ...
            # 实际的导出操作在主线程回调中按顺序执行
            self.progress.emit("准备停止Google日志收集...", 100)
    ...
    def _bugreport_only(self):
        ...
        try:
            # 1. 创建日志目录
            self.progress.emit("创建bugreport目录...", 20)
            current_date = datetime.datetime.now().strftime("%Y%m%d")
            ...
            # 2. 执行bugreport
            self.progress.emit("生成bugreport...", 50)
            ...
            # 3. 完成
            self.progress.emit("bugreport生成完成!", 100)
    ...
    def _pull_bugreport(self):
        ...
        try:
            ...
            # 2. 执行pull命令
            self.progress.emit("正在pull bugreport...", 50)
            ...
            # 3. 完成
            self.progress.emit("pull bugreport完成!", 100)
...
class PyQtGoogleLogManager(QObject):
    ...
    def start_google_log(self):
        ...
        # 创建工作线程
        self.worker = GoogleLogWorker(device, 'start')
    ...
    def stop_google_log(self):
        ...
        if not self.google_log_running:
            self.status_message.emit("Google日志未运行")
            return
        ...
        # 创建工作线程
        self.worker = GoogleLogWorker(device, 'stop', self.google_log_folder)
        self.worker.progress.connect(self._on_progress)
    ...
    def start_bugreport_only(self):
        ...
        # 创建工作线程
        self.worker = GoogleLogWorker(device, 'bugreport_only')
        self.worker.progress.connect(self._on_progress)
    ...
    def start_pull_bugreport(self):
        ...
        # 创建工作线程
        self.worker = GoogleLogWorker(device, 'pull_bugreport')
        self.worker.progress.connect(self._on_progress)
    ...
    def delete_bugreport(self):
        ...
        # 创建工作线程
        self.delete_worker = DeleteBugreportWorker(device)
        self.delete_worker.finished.connect(self._on_delete_bugreport_finished)
        self.delete_worker.start()
        self.status_message.emit("正在删除bugreport...")
    
    def _on_progress(self, message, progress):
        """进度更新"""
        self.status_message.emit(f"[Google日志] {message}")
    ...
    def _on_google_log_started(self, result):
        ...
        if result.get("success"):
            ...
            if self.adblog_manager:
                ...
                if not success:
                    self.status_message.emit("启动ADB日志失败")
            
            # 启动视频录制
            if self.video_manager:
                success = self.video_manager.start_google_recording(device, self.google_log_folder)
                if not success:
                    self.status_message.emit("启动视频录制失败")
            
            self.status_message.emit(f"Google日志收集已启动 - {self.google_log_folder}")
            self.google_log_started.emit()  # 发送启动信号
        else:
            QMessageBox.critical(None, "错误", f"启动Google日志收集失败: {result.get('error')}")
            self.status_message.emit("启动Google日志收集失败")
    ...
    def _on_google_log_stopped(self, result):
        ...
        if result.get("success"):
            ...
            # 1. 先导出视频
            self.status_message.emit("正在导出视频...")
            if self.video_manager:
                # 连接视频导出完成信号
                ...
                if not success:
                    self.status_message.emit("停止视频录制失败")
                    self._on_video_exported(folder, 0)
            else:
                self._on_video_exported(folder, 0)
        else:
            self.google_log_running = False
            QMessageBox.critical(None, "错误", f"停止Google日志收集失败: {result.get('error')}")
            self.status_message.emit("停止Google日志收集失败")
    ...
    def _on_video_exported(self, folder, count):
        ...
        # 2. 导出ADB日志
        self.status_message.emit("视频已保存，正在停止ADB日志...")
        if self.adblog_manager:
            # 连接ADB日志导出完成信号
            ...
            if not success:
                self.status_message.emit("停止ADB日志失败")
    ...
    def _on_adblog_exported(self, folder):
        ...
        # 显示进度对话框
        self.progress_dialog = QProgressDialog("正在生成bugreport...\n这可能需要2-5分钟，请耐心等待", None, 0, 0)
        self.progress_dialog.setWindowTitle("Google日志收集")
        self.progress_dialog.setCancelButton(None)  # 禁用取消按钮
        self.progress_dialog.setWindowModality(2)  # 模态对话框
        self.progress_dialog.show()
        
        # 创建bugreport工作线程
        self.bugreport_worker = BugreportWorker(self.export_device, self.export_folder)
    ...
    def _on_bugreport_finished(self, result):
        ...
        if result.get("success"):
            folder = result.get("folder")
            self.status_message.emit(f"bugreport生成完成 - {folder}")
            ...
            QMessageBox.critical(None, "错误", f"生成bugreport失败: {result.get('error')}")
            self.status_message.emit("生成bugreport失败")
            self.google_log_running = False
            self.google_log_stopped.emit()
    ...
    def _on_bugreport_error(self, error):
        ...
        QMessageBox.critical(None, "错误", f"生成bugreport失败: {error}")
        self.status_message.emit(f"生成bugreport失败: {error}")
        self.google_log_running = False
        self.google_log_stopped.emit()
    ...
    def _on_pull_bugreport_finished(self, result):
        ...
        if result.get("success"):
            folder = result.get("folder")
            self.status_message.emit(f"pull bugreport完成 - {folder}")
            
            # 打开日志文件夹
            if folder:
                os.startfile(folder)
        else:
            QMessageBox.critical(None, "错误", f"pull bugreport失败: {result.get('error')}")
            self.status_message.emit("pull bugreport失败")
    
    def _on_google_log_error(self, error):
        """Google日志错误"""
        QMessageBox.critical(None, "错误", f"Google日志操作失败: {error}")
        self.status_message.emit(f"Google日志操作失败: {error}")
    ...
    def _on_delete_bugreport_finished(self, success, error):
        ...
        if success:
            ...
            self.status_message.emit(f"删除bugreport失败: {error}")
            QMessageBox.critical(None, "错误", f"删除bugreport失败: {error}")
