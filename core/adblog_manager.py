...
class OfflineADBLogWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # 3. 检查logcat进程是否存在
            self.progress.emit("检查logcat进程...", 80)
...
class OnlineADBLogWorker(QThread):
    ...
    def __init__(self, device, log_name, online_logcat_process_ref, online_log_file_path_ref, folder=None):
        super().__init__()
        self.device = device
        self.log_name = log_name
    ...
    def run(self):
        ...
        try:
            ...
            # 2. 生成带时间的log文件名
            self.progress.emit("生成log文件名...", 40)
            current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            log_filename = f"{self.log_name}_{current_time}.txt"
            log_file_path = os.path.join(log_dir, log_filename)
            
            # 3. 启动PC端logcat进程（连线模式不需要在设备上启动logcat）
            self.progress.emit("启动PC端log输出进程...", 60)
    ...
    def _start_logcat_process(self, log_file_path, mode='w'):
        ...
        try:
            ...
            print(f"Logcat进程已启动，PID: {process.pid}, 模式: {mode}")
        except Exception as e:
            raise Exception(f"启动logcat进程失败: {str(e)}")
    ...
    def _monitor_logcat_process(self):
        ...
        while not self.stop_flag and not self.monitor_stop_flag:
            ...
            if process is None:
                print("进程引用为None，退出监控循环")
                break
            
            # 检查进程是否还在运行
            if process.poll() is not None:
                # 进程已退出，检查是否是USB断开
                print(f"Logcat进程已退出，退出码: {process.returncode}")
                ...
                if self.device not in result.stdout:
                    ...
                    # 重新启动logcat进程，追加模式
                    try:
                        self.progress.emit("重新启动logcat进程...", 70)
                        self._start_logcat_process(self.log_file_path, 'a')  # 追加模式
                        print(f"Logcat进程已重新启动，追加模式")
                        
                        # 启动成功后，发送信号通知管理器进程已更新
                        self.usb_reconnected.emit(self.device)
                    except Exception as e:
                        print(f"重新启动logcat进程失败: {e}")
                        self.error.emit(f"重新启动logcat进程失败: {str(e)}")
                        break
                else:
                    # 设备在线但进程退出，检查是否是正常停止
                    if self.normal_stop:
                        ...
                        # 设备在线但进程退出，可能是其他原因
                        print("设备在线但logcat进程退出")
                        self.error.emit("Logcat进程意外退出")
                        break
...
class ExportADBLogWorker(QThread):
    ...
    def run(self):
        ...
        try:
            # 1. 检查设备连接状态
            self.progress.emit("检查设备连接状态...", 10)
            ...
            # 检查设备是否在列表中
            if self.device not in result.stdout:
                raise Exception(f"设备 {self.device} 未连接")
            
            # 2. 检查logcat进程是否存在
            self.progress.emit("检查logcat进程...", 25)
            ...
            self.progress.emit("停止nohup logcat进程...", 40)
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                self.progress.emit("未找到txt文件，导出整个tmp目录...", 70)
...
class PyQtADBLogManager(QObject):
    ...
    def start_adblog(self, mode, log_name):
        ...
        if self.is_running:
            self.status_message.emit("ADB Log已经在运行中")
            return
    ...
    def _do_start_offline_adblog(self, device, log_name, clear_old=False):
        ...
        if clear_old:
            # 清除旧文件
            try:
                self.status_message.emit(f"正在清除设备 {device} 的旧log文件...")
                ...
                result = subprocess.run(rm_cmd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30, 
                                      creationflags=subprocess.CREATE_NO_WINDOW if hasattr(subprocess, 'CREATE_NO_WINDOW') else 0)
                if result.returncode == 0:
                    self.status_message.emit(f"已清除设备 {device} 的旧log文件")
                else:
                    print(f"警告: 清除旧文件失败: {result.stderr.strip()}")
            except Exception as e:
                print(f"清除旧文件时发生错误: {e}")
        else:
            # 用户选择保留旧文件
            self.status_message.emit(f"保留设备 {device} 的旧log文件")
        
        # 创建工作线程
        self.worker = OfflineADBLogWorker(device, log_name)
    ...
    def _on_offline_adblog_started(self, result):
        ...
        self.status_message.emit(f"离线ADB log已开启 - {result['device']} - {result['log_filename']}")
    
    def _on_offline_adblog_error(self, error):
        """离线ADB log启动失败"""
        self.status_message.emit(f"开启离线ADB log失败: {error}")
    ...
    def _start_online_adblog(self, device, log_name):
        ...
        self.worker = OnlineADBLogWorker(device, log_name, online_logcat_process_ref, online_log_file_path_ref)
    ...
    def _on_online_adblog_started_with_refs(self, result, process_ref, path_ref):
        ...
        self.current_log_filename = result['log_filename']
        self.adblog_started.emit(result['device'], result['log_filename'])
        self.status_message.emit(f"连线ADB log已开启 - {result['device']} - {result['log_filename']}")
    ...
    def _on_usb_disconnected(self, device):
        """USB断开"""
        self.status_message.emit(f"检测到USB断开，等待重连...")
        self.usb_disconnected.emit(device)
    
    def _on_usb_reconnected(self, device):
        """USB重连"""
        print(f"=== USB重连: {device} ===")
        self.status_message.emit(f"USB已重连，继续抓取log...")
        ...
        if hasattr(self, '_online_logcat_process_ref'):
            ...
            print(f"进程引用更新: {old_process.pid if old_process else 'None'} -> {self._online_logcat_process.pid if self._online_logcat_process else 'None'}")
            print(f"日志文件路径: {self._online_log_file_path}")
        else:
            print("警告: _online_logcat_process_ref 不存在")
    
    def _on_online_adblog_error(self, error):
        """连线ADB log启动失败"""
        self.status_message.emit(f"开启连线ADB log失败: {error}")
    
    def stop_online_adblog(self):
        """停止连线logcat进程（只处理连线模式）"""
        print("=== stop_online_adblog 被调用 ===")
        print(f"当前模式: {self.current_mode}")
        print(f"进程引用: {self._online_logcat_process}")
    ...
    def export_offline_adblog(self):
        ...
        print("=== export_offline_adblog 被调用 ===")
        print(f"当前模式: {self.current_mode}")
    ...
    def _stop_online_adblog(self):
        ...
        try:
            ...
            # 1. 尝试通过PC端进程对象杀掉logcat进程
            if self._online_logcat_process is not None:
                # 检查进程是否还在运行
                if self._online_logcat_process.poll() is None:
                    # 进程还在运行，终止它
                    print(f"正在终止logcat进程，PID: {self._online_logcat_process.pid}")
                    self._online_logcat_process.terminate()
                    try:
                        self._online_logcat_process.wait(timeout=5)
                        print("Logcat进程已成功终止")
                    except subprocess.TimeoutExpired:
                        print("Logcat进程终止超时，强制kill")
                        self._online_logcat_process.kill()
                        self._online_logcat_process.wait()
                    self._online_logcat_process = None
                else:
                    # 进程已经退出
                    print(f"Logcat进程已退出，退出码: {self._online_logcat_process.returncode}")
                    self._online_logcat_process = None
            
            # 2. 备用方案：通过adb命令杀掉设备上的logcat进程（PPID != 1的进程）
            print("尝试通过adb命令杀掉设备上的连线logcat进程...")
                ...
                for line in lines:
                    if 'logcat' in line:
                        ...
                        if len(parts) >= 3:
                            ...
                            if ppid != '1':
                                print(f"找到连线logcat进程，PID: {pid}, PPID: {ppid}")
                                kill_cmd = ["adb", "-s", device, "shell", "kill", pid]
                                kill_result = subprocess.run(kill_cmd, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30,
                                                          creationflags=subprocess.CREATE_NO_WINDOW if hasattr(subprocess, 'CREATE_NO_WINDOW') else 0)
                                if kill_result.returncode == 0:
                                    print(f"成功杀掉连线logcat进程 PID: {pid}")
                                else:
                                    print(f"杀掉进程 PID {pid} 失败: {kill_result.stderr.strip()}")
            
            # 3. 打开日志文件所在目录
            if self._online_log_file_path:
                log_dir = os.path.dirname(self._online_log_file_path)
                print(f"打开日志文件夹: {log_dir}")
                os.startfile(log_dir)
                self.status_message.emit("连线ADB log已停止并保存")
            else:
                print("日志文件路径为空")
                self.status_message.emit("连线ADB log已停止")
            ...
            print(f"停止连线ADB log异常: {str(e)}")
            import traceback
            traceback.print_exc()
            self.status_message.emit(f"停止连线ADB log失败: {str(e)}")
            self.is_running = False
            self.adblog_stopped.emit()
            self.online_mode_stopped.emit()
    ...
    def _export_offline_adblog(self, device):
        """导出离线adb log"""
        # 创建工作线程
        self.worker = ExportADBLogWorker(device)
    ...
    def _on_offline_adblog_exported(self, result):
        ...
        self.status_message.emit(f"离线ADB log已导出 - {result['device']}")
    
    def _on_offline_adblog_export_error(self, error):
        """离线ADB log导出失败"""
        if "logcat进程不存在" in error:
            self.status_message.emit("logcat进程不存在，log抓取异常")
        else:
            self.status_message.emit(f"停止并导出离线ADB log失败: {error}")
    ...
    def _on_google_adblog_started(self, result, folder, log_file_path):
        ...
        if result.get("success"):
            ...
            self.status_message.emit(f"Google ADB日志已启动: {result['device']}")
        else:
            self.status_message.emit(f"启动Google ADB日志失败: {result.get('error')}")
    
    def _on_google_adblog_error(self, error):
        """Google ADB日志错误"""
        self.status_message.emit(f"Google ADB日志操作失败: {error}")
    ...
    def stop_and_export_to_folder(self, device, folder):
        ...
        if not self.is_running:
            self.status_message.emit("ADB Log未运行")
            return False
        ...
        try:
            ...
            self.status_message.emit(f"ADB日志已停止，准备生成bugreport...")
            
            return True
            
        except Exception as e:
            self.status_message.emit(f"停止ADB日志失败: {str(e)}")
            self.is_running = False
            self.adblog_exported.emit(folder)
            return False
