...
class TelephonyManager:
    ...
    def execute_telephony_commands(self, progress_var=None, status_label=None, progress_dialog=None):
        """执行Telephony相关的adb命令"""
        try:
            # 获取验证后的设备
            device = self.device_manager.validate_device_selection()
            ...
            total_commands = len(self.telephony_commands)
            
            # 更新状态
            if status_label:
                status_label.config(text="正在准备Telephony命令...")
            ...
            # 更新状态
            if status_label:
                status_label.config(text="正在执行Telephony命令...")
                progress_dialog.update()
            ...
            # 完成所有命令
            if status_label:
                status_label.config(text="所有命令执行完成")
            
            return True
            
        except subprocess.TimeoutExpired:
            messagebox.showerror("错误", "命令执行超时")
            return False
        except Exception as e:
            messagebox.showerror("错误", f"执行Telephony命令失败: {str(e)}")
            return False
    ...
    def reboot_device(self):
        ...
        try:
            ...
            if result.returncode == 0:
                messagebox.showinfo("成功", f"设备 {device} 正在重启...")
                return True
            else:
                messagebox.showerror("错误", f"重启设备失败: {result.stderr}")
                return False
                
        except subprocess.TimeoutExpired:
            messagebox.showerror("错误", "重启命令超时")
            return False
        except Exception as e:
            messagebox.showerror("错误", f"重启设备失败: {str(e)}")
            return False
    
    def enable_telephony_logs(self):
        """启用Telephony日志的主函数"""
        try:
            # 1. 检查设备连接
            ...
            def on_success(result):
                ...
                if messagebox.askyesno("重启确认", 
                    ...
                    messagebox.showinfo("完成", "Telephony日志设置完成！\n请手动重启设备以使设置生效。")
            
            def on_error(error):
                messagebox.showerror("错误", f"设置Telephony日志失败: {str(error)}")
            ...
            messagebox.showerror("错误", f"启用Telephony日志失败: {str(e)}")
            return False
