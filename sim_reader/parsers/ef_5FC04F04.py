...
def parse_data(raw_data: list) -> list:
    ...
    for index, data in enumerate(raw_data):
        results.append({"5GS non-3GPP Access NAS Security Context": data})
    
    return results
def encode_data(user_data: str,ef_file_len_decimal: int) -> str:
    try:
        encode_userdata = user_data.get('5GS non-3GPP Access NAS Security Context', '')
        if len(encode_userdata) <= ef_file_len_decimal * 2:
            ...
            logging.debug("5GS non-3GPP Access NAS Security Context longer than current EF (%d >%d),resize in write_data()",
