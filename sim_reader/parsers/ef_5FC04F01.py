...
def parse_data(raw_data: str) -> None:
    ...
    results.append({"5G-GUTI": guti, "Last visited registered TAI in 5GS for 3GPP access": last_visited_registered_tai, "5GS update status for 3GPP access": update_status})
    return results
    

def encode_data(user_data: str,ef_file_len_decimal: int) -> str:
    try:
        encoded_guti = user_data.get('5G-GUTI', '')
        if len(encoded_guti) != 26: 
            raise ValueError("5G-GUTI长度不正确")
        encoded_last_visited_registered_tai = user_data.get('Last visited registered TAI in 5GS for 3GPP access', '')
        if len(encoded_last_visited_registered_tai) != 12:
            raise ValueError("Last visited registered TAI in 5GS for 3GPP access长度不正确")
        encoded_update_status = user_data.get('5GS update status for 3GPP access', '')
        if len(encoded_update_status) != 2:
            raise ValueError("5GS update status for 3GPP access长度不正确")
