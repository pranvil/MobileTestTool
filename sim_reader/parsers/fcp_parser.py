...
def parse_fcp_data(fcp_data):
    ...
    if not fcp_data.startswith("62"):
        logging.warning("无效的FCP数据格式，应以'62'开头: %s", fcp_data)
        return fcp_data
    ...
    logging.debug("开始解析FCP数据: %s", fcp_data)
    ...
    while index < data_length: 
        ...
        # logging.debug("解析FCP字段 - TAG: %s, 长度: %d, 值: %s", tag, length, value)
    ...
    logging.info("FCP解析完成: %s", ef_info)
    return ef_info
...
def parse_file_descriptor_T(value: str):
    ...
    """
    if len(value) < 1:
        logging.error("文件描述符长度不足: %s", value)
        raise ValueError("值的长度不足，无法解析文件描述符")

    # logging.debug("开始解析2字节文件描述符: %s", value)
    ...
    logging.debug("文件可访问性: %s", file_accessibility)
...
def parse_file_descriptor_L(value: str):
    ...
    # logging.debug("开始解析5字节文件描述符: %s", value)
    ...
    logging.debug("记录器信息: %s, 记录长度: %s", result[-2], result[-1])
...
def parse_life_cycle_status(value):
    ...
    Returns:
        str: 生命周期状态的描述字符串
    """
    # logging.debug("开始解析生命周期状态: %s", value)
    
    # 转换为整数并验证范围
    life_cycle_byte = int(value, 16) 
    if not (0 <= life_cycle_byte <= 255):
        logging.error("无效的生命周期状态值: %s", value)
