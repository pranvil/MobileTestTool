...
def parse_dialing_number(hex_str: str) -> str:
    ...
    if swapped_number.startswith('A'):
        swapped_number = '*' + swapped_number[1:]  # 替换第一位为'*'
...
def encode_data(user_data: dict, ef_file_len_decimal: int) -> str:
    ...
    if user_data:
        ...
        if dialing_number == "":
            ...
            last_2_bytes = "FFFF"
        elif dialing_number[0] == '*' and all(c.isdigit() for c in dialing_number[1:]):  # 允许首位为 '*'
            dialing_number = 'A' + dialing_number[1:]  # 将 '*' 替换为 'A'
            ...
            return "error: Dialing Number must be numeric or start with '*' followed by digits"
